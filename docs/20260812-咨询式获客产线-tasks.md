...
## ⛔ 首轮实测结论：渠道假设不成立（2026-08-13）

技术链路全部验证通过（抓取 → L1 → 主页 → L2 → 诊断书），但**渠道本身
捞不到目标客户**。三个原因叠加，每一个单独都能忍，叠在一起有效线索≈0：
...
小红书主站用户主体在国内，搜中文建站词吸引的自然是国内做独立站的人。
产量最高的两个词（「帮商家做网站」22 条、「独立站搭建服务」33 条）全是国内。

### 2. 热度悖论：有评论的没客户，有客户的没评论

| 笔记类型 | 赞 | 评论 | 评论区是谁 |
|---|---|---|---|
| 爆款讨论帖 | 4597 | 多 | 开发者聊 agent 技术 |
| 面向国内的同行帖 | 250–489 | 多 | 国内做独立站的人 |
| **面向北美的同行帖** | **6** | **0** | — |

实测直接抓「FlamingoTech北美商家帮」那篇（6 赞，最对口的一篇）：
**评论区 0 条**。北美市场在小红书上的同行帖，流量本身就小。
...
```
@寒武纪网络科技工作室  正经工作室报价都接不到单了…
@国科中元软件        这边是做小程序、软件开发的科技公司…
@深圳思亿欧独立站服务商 别在贪小便宜建站了…
@Danny_             我报1w，你先报2.98w上去…完事我们五五分
```

同行推广帖的评论区，主要参与者是**其他同行**在抢客和讨论价格。
L1 能正确识别（实测 @品牌出海建站 判 5 分 + risk_flags:["同行"]），
但这意味着抓取的风控预算大部分花在了注定被过滤的内容上。

### 技术侧是好的，而且核心能力得到了验证

「昵称会骗人，笔记标题不会」这条假设，实测撞到三次，每次都成立：

| 昵称 | 主页笔记 | 真相 |
|---|---|---|
| 造梦设计师01 | 方逸伦、柳眼、雪天浪漫 | 追星粉丝号 |
| 寒武纪网络科技工作室 | 足控、兔子、保洁、猫猫打架、荔枝 | 个人号，无任何案例 |
| Bing Fitness（README 旧实测） | — | 昵称让判断从「其他」变「预约系统」 |

L2 的诊断质量也验证了差异化能力。对 @寒武纪网络科技工作室 的输出：

> **他以为的问题**：低价模板把价格打烂了
> **真正的瓶颈**：他没有任何可验证的交付证据——主页翻不到一个案例、一张
> 后台截图、一条客户反馈，询价的人只能用价格这一个维度做比较，于是必然输给模板

这正是设计要求的「瓶颈必须和他自己以为的需求不同」。**能力是有的，投错了地方。**

### 下一步需要决策，不要在没定方向前继续跑抓取

每跑一轮都要花风控预算，而当前词表的产出注定是废品。
...
**新增 77 条，去重跳过 1 条**（跨词重复的商家被 uid 合并挡住了）。
对比旧渠道同样五个词：55 条线索，北美 1 条，还是同行。

### L1：命中率从 6% 到 83%

```
@May Blossom Centre      → ✅ 62 分 | 预约系统
@emo喵                   → ·  30 分（标题只有表情，判为非商家 —— 对的）
@Shine nail studio       → ✅ 85 分 | 预约系统
@NaNail_Studio_A         → ✅ 85 分 | 预约系统
@97爱思米米              → ✅ 65 分 | 预约系统
@多伦多fancy美睫美甲师   → ✅ 88 分 | 预约系统
```

6 条真分析 5 条合格。旧渠道是 31 条里 2 条。

**全部判为「预约系统」而不是「官网」** —— 美甲美睫生意靠排期，这正是
「同一条线索别人卖 500，我们卖 5000 + 月费」那句话在代码里的落点。

### 这轮又修掉的两个坑

| 症状 | 根因 | 修法 |
|---|---|---|
| `--limit 8` 跑出「0/8，跳过 8」 | 地域过滤在 AI 循环里逐条穿越，积压的国内线索把配额耗光 | `skipNonTargetRegion` 一条 SQL 批量清；limit 只算真实 AI 调用 |
| `is_reply` 一直没进库且不报错 | better-sqlite3 对多余命名参数**静默容忍**，字段直接消失 | 补列；并在 migrate 注释里写明加字段要同时改两处 |

第二个坑值得记牢：**测试测的是函数返回值，不是数据库**，所以断言照过。
这类 bug 只能靠实际查库发现。


### L2：5/5 产出诊断书，且产品线判断不套模板

同样是美甲/美睫店，四个判「预约系统」，一个判「会员积分」：

| 商家 | 产品线 | 模型给的理由 |
|---|---|---|
| Shine nail studio | 预约系统 | 档期只在私信和脑子里，美甲一单坐 1–2 小时，撞一次就丢客 |
| 97爱思米米 | 预约系统 | 上门服务的档期、地址、款式全靠私信对，问的人多落到时间上的少 |
| 多伦多fancy美睫美甲师 | 预约系统 | 一个人做手，那两三小时私信没人回，38 引来的流量在私信里漏掉 |
| May Blossom Centre | 预约系统 | 技师×时长×时段三重排期，主打午休时段本来就窄 |
| **NaNail_Studio_A** | **会员积分** | **「同一条 8月58刀 发了三四遍」→ 月月靠降价拉新客，而美甲有 3–4 周天然复购周期，缺的是召回机制** |

最后一条是这条产线价值的最好证明：模型从「同一条促销发了三四遍」这个细节，
推出了她自己都没意识到的经营问题。**这就是「别人卖 500，我们卖 5000 + 月费」
那句话在代码里的落点。**

还有一个「必须看主页」的新例证：`May Blossom Centre` 是从「多伦多 美甲」
搜出来的，但主页全是足疗、按摩、SPA —— **她根本不是美甲店**。
只看搜索词和标题会判错，拉一次主页真相就出来了。

### 首批诊断暴露的文案问题（已修）

这五个字段是一字不改印在发给客户那页纸上的，首批两条都违反了：

- 第三人称：「**他**却把力气全花在月度促销拉新上」
- 推理过程漏进客户页：「昵称 NaNail_Studio_A，笔记全是本人一人的款式实况图，
  **无员工/分店痕迹**」—— 这是给我们看的判断依据，印在客户页上像监视报告

已在 prompt 里写死（第二人称、不写推理、business_size 限四词、industry ≤15 字），
并把这几条约束写成测试断言 —— 单元测试测不到文案质量，只能钉住 prompt 本身。

### 完整链路已通

```
77 个北美商家 → L1 命中 83% → 5 个主页 0 扑空 → 5 份诊断书 0 失败
```


---

## 对照验证：产品线判断不是模板（2026-08-13）

首轮 5 份诊断全判「预约系统」，怀疑是模板化。抓三个非排期品类做对照：

| 品类 | 样本 | L1 判定 | 结论 |
|---|---|---|---|
| 美甲 / 美睫 / SPA | 5 | 全部**预约系统** | 靠排期，对 |
| 地产经纪 | 4 | 全部**官网**（80/78/55 分 + 1 条非商家 25 分） | 展示型生意，对 |
| 华人餐厅 | 4 | 全部低分 25–35 | **词选错了**，见下 |

**产品线判断不是模板** —— 排期生意判预约，展示型生意判官网，两类泾渭分明。

### 但发现一个品类适配问题

搜「温哥华 华人 餐厅」抓到的是 @umi小章鱼、@Fituna 这类**食客的探店笔记**，
不是餐厅老板。L1 正确归进「探店号/测评号」那一档，判断没问题，是词错了。

背后是真实的品类差异：**美甲美睫师靠小红书获客（作品即广告），餐饮不靠** ——
餐厅老板在 Google / Yelp 上经营，小红书上是顾客在替他发内容。

后果：**「会员积分」这条产品线在小红书上不好找客户**。餐饮零售回头客生意
是它的主场，而那批人不在这个平台自我推广。要么换平台（Google Business，
看板上 c-3b5669 Local Falcon / c-73a943 localrank 都是这个方向），
要么找到餐饮业主自己会发的内容形态 —— 后者未验证，已移入
`UNVERIFIED_KEYWORDS`，不凭猜想往主词表里加。

### 词表据此重排

主词表只留已验证高产的两类：
- **美业 / 健身 / 宠物**（技师本人发作品）→ 预约系统
- **地产 / 律师 / 装修 / 搬家 / 会计**（本人经营账号）→ 官网
